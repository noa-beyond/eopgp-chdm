...
def define_G(net_G, init_type="normal", init_gain=0.02, gpu_ids=None, input_nc=3):
    if net_G == "base_resnet18":
        net = ResNet(input_nc=input_nc, output_nc=2, output_sigmoid=False)

    elif net_G == "base_transformer_pos_s4":
    ...
    elif net_G == "base_transformer_pos_s4_dd8":
        net = BASE_Transformer(
            input_nc=input_nc,
            output_nc=2,
            token_len=4,
    ...
    elif net_G == "base_transformer_pos_s4_dd8_dedim8":
        net = BASE_Transformer(
            input_nc=input_nc,
            output_nc=2,
            token_len=4,
        ...
        raise NotImplementedError("Generator model name [%s] is not recognized" % net_G)
...
class ResNet(nn.Module):
    def __init__(
        ...
        if backbone == "resnet18":
            self.resnet = resnet_model.resnet18(
                pretrained=True, replace_stride_with_dilation=[False, True, True]
            )
        elif backbone == "resnet34":
            self.resnet = resnet_model.resnet34(
                pretrained=True, replace_stride_with_dilation=[False, True, True]
            )
        elif backbone == "resnet50":
            self.resnet = resnet_model.resnet50(
                pretrained=True, replace_stride_with_dilation=[False, True, True]
            )
            expand = 4
        else:
